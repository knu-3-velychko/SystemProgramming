...
public class PageFault {
    ...
    public static void replacePage(Vector mem, int virtPageNum, int replacePageNum, ControlPanel controlPanel) {
        System.out.println(replacePageNum);
        ...
        for (int i = 0; i < mem.size(); i++) {
            Page tmp = (Page) mem.elementAt(i);
            if (!tmp.secondChance && tmp.physical != -1) {
                removePage(mem, i, replacePageNum, controlPanel);
                tmp.secondChance = true;
                return;
            } else
                tmp.secondChance = false;
        }
    }

    private static void removePage(Vector mem, int oldestPage, int replacePageNum, ControlPanel panel) {
        Page page = (Page) mem.elementAt(oldestPage);
        Page nextpage = (Page) mem.elementAt(replacePageNum);

        panel.removePhysicalPage(oldestPage);
        nextpage.physical = page.physical;
        panel.addPhysicalPage(oldestPage, replacePageNum);
        page.inMemTime = 0;
